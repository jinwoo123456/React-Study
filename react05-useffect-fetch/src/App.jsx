...
function MyCommunication(props) {
   ...
   * JSON의 포멧에 따라 다라질 수 있으므로 확인 후 설정한다.
   */
  var [myJSON, setmyJSON] = useState({ results: [] });

  //해당 UI의 랜더링이 끝난 후 API의 정보를 얻어온다.
  useEffect(function () {
    ...
    return () => {
      console.log("#Life", `useEffect실행==> 컴포넌트 언마운트(사라질때 실행)`);
    };
  }, []);
  /**두번째 인자인 의존성배열에 빈 배열을 설정했으므로, 최초 한번만
   * 실행된다.이 부분의 설정이 없으면 무한루프에 빠지게 된다.
   */
  let trTag = [];
  ...
  return (
    <>
      ...
      <MyCommunication
        onProfile={(sData) => {
          console.log(sData);
          /**정보 출력을 위한 문자열은 백택 기호를 이용해서 + 기호 없이
           * 연결할 수 있다.
           */
